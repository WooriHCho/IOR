...
void write_server(int client, char *message) {
    ...
    messageArr[strlen(messageArr)] ='\0';
    if(strcmp(messageArr,"start") == 0)
    {
	msg1 =1;
    }else if(strcmp(messageArr, "end") != 0)
    {
	msg1 = 0;
    }
}
...
void MOTORcontrol(int motor, int angle){
	printf("motor action\n");
	pwmWrite(motor,angle);
	delay(1000);
}
int main()
{
